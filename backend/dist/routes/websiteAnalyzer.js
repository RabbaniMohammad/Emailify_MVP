...
router.post('/analyze-website', async (req, res) => {
    try {
        ...
        jet_logger_1.default.info(`📥 Received website analysis request for: ${url}`);
        // Validate URL
        const validation = (0, websiteAnalyzer_1.validateUrl)(url);
        if (!validation.valid) {
            jet_logger_1.default.info(`❌ Invalid URL: ${validation.error}`);
            return res.status(400).json({ error: validation.error });
        }
        // Analyze website
        const brandDNA = await (0, websiteAnalyzer_1.analyzeWebsite)(validation.normalizedUrl);
        jet_logger_1.default.info(`✅ Website analysis successful for: ${url}`);
        res.json(brandDNA);
    }
    catch (error) {
        jet_logger_1.default.info('❌ Website analysis error: ' + error.message);
 ...
 */
router.post('/brand-dna-to-prompt', async (req, res) => {
    try {
        const { colors, images, content, fonts, templateStyle, url } = req.body;
        jet_logger_1.default.info(`📥 Received brand DNA to prompt request - Colors: ${colors?.length || 0}, Images: ${images?.length || 0}, Content: ${content?.length || 0}, Style: ${templateStyle}, URL: ${url}`);
...
COLORS (Selected):
${promptData.colors.length > 0 ? promptData.colors.map((c) => `- ${c}`).join('\n') : '- Use website colors'}
...
${promptData.images.length > 0 ? promptData.images.map((img, i) => `${i + 1}. ${img}`).join('\n') : 'No images selected'}

CONTENT SNIPPETS (Brand Voice):
${promptData.content.length > 0 ? promptData.content.map((c) => `- "${c}"`).join('\n') : 'Use professional tone'}

FULL CONTENT SECTIONS (Detailed Information):
${promptData.contentSections.length > 0 ? promptData.contentSections.map((section, i) => `${i + 1}. ${section.heading ? section.heading + ' (' + section.context + ')' : section.context}
   Paragraphs:
   ${section.paragraphs.map((p) => `   - ${p}`).join('\n')}
   ${section.listItems && section.listItems.length > 0 ? `\n   List Items:\n   ${section.listItems.map((li) => `   • ${li}`).join('\n')}` : ''}`).join('\n\n') : 'No detailed content sections extracted'}
...
PRODUCTS (if any):
${promptData.products.length > 0 ? promptData.products.map((p, i) => `${i + 1}. ${p.name}${p.price ? ` - ${p.price}` : ''}
   ${p.description ? `   Description: ${p.description}` : ''}
   ${p.image ? `   Image: ${p.image}` : ''}`).join('\n') : 'No products extracted'}

TESTIMONIALS:
${promptData.testimonials.length > 0 ? promptData.testimonials.map((t, i) => `${i + 1}. "${t.text}"${t.author ? ` - ${t.author}` : ''}${t.company ? `, ${t.company}` : ''}`).join('\n') : 'No testimonials'}
...
${promptData.ctas.length > 0 ? promptData.ctas.map((cta, i) => `${i + 1}. "${cta.text}" → ${cta.url}`).join('\n') : 'No CTAs selected'}
...
Generate a complete, detailed prompt that will create a professional MJML email template incorporating ALL the brand DNA elements, especially the CTAs with their actual links, full content sections, and authentic messaging.`
        ...
        jet_logger_1.default.info(`✅ Prompt generated successfully`);
        res.json({
            success: true,
        ...
        jet_logger_1.default.info('❌ Brand DNA to prompt error: ' + error.message);
