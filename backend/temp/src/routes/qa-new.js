...
function restoreEmojisInChange(change, originalTaggedContent) {
...
function extractContentWithCheerio(html) {
...
function createTaggedContent(extracted) {
    const tagged = [];
...
function extractContextFromHtml(html, targetText, contextChars = 50) {
...
function extractContext(allTagged, currentItem, contextChars = 40) {
...
async function getContentImprovementsForBatch(batch, allTagged, taskType = 'grammar') {
...
async function getContentImprovements(taggedContent, taskType = 'grammar') {
...
function checkForNewFacts(original, replacement) {
    const origNumbers = original.match(/\d+/g) || [];
    const newNumbers = replacement.match(/\d+/g) || [];
    ...
    const origEmails = original.match(emailRegex) || [];
    const newEmails = replacement.match(emailRegex) || [];
    ...
    const newUrls = replacement.match(urlRegex) || [];
...
function validateChange(change, original, taskType = 'grammar') {
    const failed = [];
...
function applyChangesToHtml(originalHtml, taggedContent, changes, taskType = 'grammar') {
    const applied = [];
    const skipped = [];
    for (const change of changes) {
...
function applyChangesToHtmlById(originalHtml, taggedContent, approvedChanges) {
...
function replaceTextInHtml($, tagged, change) {
...
router.post('/:id/seo', async (req, res) => {
    try {
        const templateId = String(req.params.id);
        const html = String(req.body?.html || '').trim();
        if (!html) {
...
router.post('/:id/test-extraction', async (req, res) => {
    try {
        const html = String(req.body?.html || '').trim();
        if (!html) {
